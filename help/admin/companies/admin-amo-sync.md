...
Par défaut, toutes les entreprises synchronisent les données avec [!DNL Adobe Media Optimizer] ([!DNL AMO]). Dans le [!UICONTROL Admin UI], chaque conteneur de l’entreprise dispose d’une source de données qui gère ce processus. Cette source de données est [!UICONTROL Adobe AMO] ([!UICONTROL ID] 411). Cliquez sur une ligne de conteneur (sous l’onglet [!UICONTROL Containers]) pour qu’une entreprise sélectionnée désactive cette synchronisation par défaut ou pour ajouter et supprimer d’autres sources de données au processus de synchronisation [!DNL AMO].
...
| Activé (quelle que soit la version du service d’ID) | Une source de données se synchronise avec [!DNL AMO] avec le service d’ID version 2.0 (ou ultérieure) lorsque : <ul><li>La source de données apparaît dans la liste [!UICONTROL Selected Data Sources].</li><li>La case à cocher [!DNL AMO] *est* sélectionnée.</li></ul> |
